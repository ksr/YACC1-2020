...
int parseStatement() {
    switch (tokArr[tokenIndex].type) {
        ...
        case FOR:
        {
            ...
            for_stack[for_stack_ptr].line_after_for = 0;
            for_stack_ptr++;
            printf("For %c %d %d\n",*var.value, startVal, toVal);
...
static void
for_statement(void) {
    Token for_variable, to;

    eat(FOR);
    // orig for_variable = tokenizer_variable_num();
    for_variable = eat(ID);
    ...
    int val = parseExpression();
    insertVariable(for_variable.value, val, NUM)
