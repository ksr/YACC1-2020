...
int main(int argc, char *argv[]) {
    ...
    if (argc == 2) {
        ...
        while (pc < i) {
            ...
            for(int t=0;tokArr[t].type != ENTER;t++)
                ;
                 //printf("%d %d\n",t,tokArr[t]);
...
#define new
        ...
        case FOR:
        {
            //printf("For - for stack ptr %d\n",for_stack_ptr);
            for_stack_ptr++;
            Token var;
            eat(FOR);
            var = eat(ID);
            eat(ASSIGN);
            int startVal = parseExpression();
            insertVariable(var.value, startVal, NUM);
            eat(TO);
            int toVal = parseExpression(); // add step later
            //printf("For  %d %d\n", startVal, toVal);
            for_stack[for_stack_ptr].for_variable = var.value; //fix
            //printf("For  %d %d\n", startVal, toVal);
            for_stack[for_stack_ptr].to=toVal;
            //printf("For  %d %d\n", startVal, toVal);
            for_stack[for_stack_ptr].line_after_for = pc;
            //printf("For %c %c %d %d\n",*var.value, *for_stack[for_stack_ptr].for_variable, startVal, toVal);
            //printf("For %d %d\n", startVal, toVal);
        ...
        case NEXT:
        {
            //printf("Next\n");
            eat(NEXT);
            Symbol *sVar = getSymbol(for_stack[for_stack_ptr].for_variable);
            //printf("Next %c %d\n",*sVar->variable,sVar->value);
            sVar->value++;
            //printf("Next %c %d %d\n",*sVar->variable,sVar->value,for_stack[for_stack_ptr].to);
            if(sVar->value < for_stack[for_stack_ptr].to)
                pc=for_stack[for_stack_ptr].line_after_for;
